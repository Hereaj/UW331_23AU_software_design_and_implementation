...
export const RowTableElems = (props: {row: Row, key: number}): JsxList => {
  if (props.row === "rnil") {
    ...
    const squareElement = SquareTableElem({square: props.row.hd, key: props.key});
    const restofElement = RowTableElems({row: props.row.tl, key: props.key + 1})
    return jcons(squareElement, restofElement);
  }
}
...
export const QuiltTableElems = (props: {quilt: Quilt, key: number}): JsxList => {
  if (props.quilt === "qnil") {
    ...
    const rowElement = RowTableElem({row: props.quilt.hd, key: props.key});
    const restofRow = QuiltTableElems({quilt: props.quilt.tl, key: props.key + 1});
    return jcons(rowElement, restofRow);
  }
}
