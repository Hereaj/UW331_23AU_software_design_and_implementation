...
const getQuilt = (pattern: string, rows: number, color?: Color): Quilt => {
  switch (pattern) {
...
const rows: number = getRows(params);
...
if (!params.has("pattern")) {
  ...
  if (pattern === undefined) {
    ...
    try {
      ...
      const symmetrize_result = (params.has("symmetrize")) ? symmetrize(result) : result
      const final_result_with_table = (params.has("table")) ? <React.StrictMode><QuiltTableElem quilt={symmetrize_result}/></React.StrictMode> : <React.StrictMode><QuiltElem quilt={symmetrize_result}/></React.StrictMode>

      root.render(final_result_with_table);
