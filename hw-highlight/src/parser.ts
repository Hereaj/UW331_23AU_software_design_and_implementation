 ...
 * @param chars List of characters representing the input text.
 * @returns List of Highlight records describing the text.
 */
export const findHighlights = (chars: List<number>): List<Highlight> => {
  if (chars === nil) {
    return nil;
  } else {
    const highlight_info = getNextHighlight(chars);

    // char is only text
    if (highlight_info === undefined) {
      return cons({color: 'white', text: compact(chars)}, nil);
    } else {
      const [front_text, highlights, rear_list] = highlight_info;
      if (front_text !== '') {
        return cons({color:'white', text: front_text}, cons({color:highlights.color, text:highlights.text}, findHighlights(rear_list)));
      } else {
        return cons({color:highlights.color, text:highlights.text}, findHighlights(rear_list));
      }
    }
  }
